...
Generate a changeset with `pnpm changeset` in the repository root when changing the public surface of packages/cli, packages/js-sdk, or packages/python-sdk; internal scripts, devtools, and tests don't need one. Each changeset must only name packages from one release group (E2B SDK, CLI, Code Interpreter SDK, Desktop SDK; see .changeset/README.md); write one changeset per group.
