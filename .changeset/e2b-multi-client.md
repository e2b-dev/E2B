...
`Template` is now the `TemplateBase` class made callable without `new`, so `Template(...)`, `new Template(...)`, the statics and `instanceof` all keep working, and per-client template surfaces are plain subclasses.
