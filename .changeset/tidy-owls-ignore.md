...
The files hash changes once for affected `copy()` steps. Files inside ignored directories are no longer uploaded, and copying a path inside an ignored directory now fails with "No files found" unless a `!` pattern re-includes it. That error now points at `.dockerignore` and `fileIgnorePatterns`.
