...
class RepoAgent(AgentBase):
    ...
    def _create_prompt(
        ...
        system_template = chevron.render(
            "\n\n".join(
                [
                    RepoAgent._get_prompt_part(config.prompt, "system", "prefix"),
                    "\n".join([f"{tool.name}: {tool.description}" for tool in tools]),
                    RepoAgent._get_prompt_part(config.prompt, "system", "suffix"),
                ]
            ),
            instructions,
        )
        ...
        human_template = chevron.render(
            "\n\n".join(
                [
                    RepoAgent._get_prompt_part(config.prompt, "user", "prefix"),
                ]
            ),
            instructions,
        )
    ...
    async def _run(self, instructions: Any):
        ...
        try:
            ...
            # TODO: Handle rootdir for the repo
            await playground.checkout_repo(instructions["repo_url"])
