...
class StripeAgent(AgentBase):
    ...
    def _create_prompt(
        ...
        system_template = chevron.render(
            "\n\n".join(
                [
                    StripeAgent._get_prompt_part(config.prompt, "system", "prefix"),
                    "\n".join([f"{tool.name}: {tool.description}" for tool in tools]),
                    StripeAgent._get_prompt_part(config.prompt, "system", "suffix"),
                ]
            ),
            instructions,
        )
        ...
        human_template = chevron.render(
            "\n\n".join(
                [
                    StripeAgent._get_prompt_part(config.prompt, "user", "prefix"),
                ]
            ),
            instructions,
        )
