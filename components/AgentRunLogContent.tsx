...
function AgentRunLogContent({ log: initialLog }: Props) {
  ...
  const isRunning =
    initialLog.deployments.enabled &&
    !isRunOlderThanTimeout &&
    runSteps.length === 1
  const pr = useMemo(() => {
    ...
    <main className="overflow-hidden flex flex-col max-h-full flex-1 rounded-md">
      ...
      <div className="flex-1 flex space-x-2 items-start justify-start overflow-hidden my-4">
        <Splitter
          ...
          minWidths={[120, 120]}
        >
          <AgentDeploymentStepsList steps={log.content as any} />
          <AgentRunLogDetail step={selectedStep} />
        </Splitter>
      </div>
    </main>
  )
