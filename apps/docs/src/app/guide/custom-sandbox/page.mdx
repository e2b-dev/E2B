...
<Note>
We support only Debian-based base images at the moment.
</Note>
...
<Note>
[Pro users](/pricing) can use the `--cpu-count=` ([docs](/cli/commands#template-build)) and `--memory-mb=` ([docs](/cli/commands#template-build)) flags to customize the sandbox compute.
Read more about the compute [here](/sandbox/compute).
</Note>
