...
Follow our simple guides to let the OpenAI's assistant use the E2B sandbox to download files, use a custom cloud browser, use a terminal, write, run, and debug code, and more.
- [Python](#python)
...
An assistant action is a regular Python function that takes the sandbox object as the first argument and a dictionary with arguments as the second. The assistant will be able to call this function to complete its task when it runs.
