...
An assistant action is a regular Python function that takes the sandbox object as the first argument and a dictionary with arguments as  second. The assistant will be able to call this function to complete its task when it runs.
The action can do be pretty much anything you want - running code, downloading files, analyzing data, for example running a terminal command.
...
Now we need to make sure the sandbox knows about our actions we defined above. We'll use the `add_action` method.
<CodeGroup isFileName title="main.py" isRunnable={false}>
```py
# Import our action
...
The E2B sandbox will automatically call the correct actions and returns correctly formatted output object (called `outputs` in the code below) that you can submit back to the assistant.
...
### `sandbox.addAction()`
Use the `addAction()` method to register a JavaScript function as an runnable action for the AI assistant and the LLM.
...
### 3. Add action to sandbox
Now we need to make sure the sandbox knows about our actions we defined above. We'll use the `addAction` method.
<CodeGroup isFileName title="index.ts" isRunnable={false}>
```js
// Import our action
