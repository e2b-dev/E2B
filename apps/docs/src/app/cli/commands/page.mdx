...
<Options>
  <Option name="level" type="-l, --level">
  ...
  <Option name="services" type="--services">
    Specify services for printing logs — allowed values are `process`, `filesystem`, `terminal`, `network`.
    You can specify multiple services by separating them with a space.
