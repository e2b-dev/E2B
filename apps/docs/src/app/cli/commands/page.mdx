...
<Note>
Running `e2b template spawn` without specifying a template with the `[template]` argument will spawn sandbox defined by the `e2b.toml` config.
</Note>
...
Immediatelly kill a running sandbox.
