...
export default function Settings({ user, apiKey }: Props) {
  ...
  const items = [
    ...
    },
    {
      title: 'API Key',
      value:
        apiKey.substring(0, 6) + '*'.repeat(18) + apiKey.substring(apiKey.length - 8),
      icon: copyIcon,
      description:
        'Using e2b cloud enviroments requires an API Key for every request. You can use the keys below in the e2b SDK library.',
    },
  ]
  const onClick = () => {
