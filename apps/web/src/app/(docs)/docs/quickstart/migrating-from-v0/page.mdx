...
```bash
grit apply e2b_v0_to_v1_js
```

#### For Python:

```bash
grit apply e2b_v0_to_v1_py
```

### Step 5: Review changes

Once the migration is applied, review the changes for possible mistakes, including code that was broken (false positives) or outdated code that was not fixed (false negatives) by the migration.
