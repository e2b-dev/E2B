...
Sometimes you may want to upload files to the sandbox from a different environment, that is not authorized, like a user's browser.
In this case, you can use pre-signed URLs to upload files securely.
...
// Create a pre-signed URL for file upload with 10 seconds expiration
...
// Upload file with pre-signed URL (this can be executed from any environment such as clients browser)
const form = new FormData()
form.append('file', 'file content')

await fetch(publicUploadUrl, { method: 'POST', body: form })

// File is now available in the sandbox and can be read
