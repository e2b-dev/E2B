...
When creating a sandbox you now **specify the sandbox timeout** — that is how long the sandbox will exist. If you don't specify the timeout the sandbox will be closed after 5 minutes.
...
You can still use `.kill()` to kill the sandbox immediately if you are sure you won't need it anymore.
...
# Now
sandbox = Sandbox(template='my-template', timeout=300)
```
</CodeGroup>
