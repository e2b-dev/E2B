...
- **Secure by default**: Sandboxes now default to `secure=True`
...
Sandboxes are now **secure by default** (`secure=True`).

This shouldn't affect you, but if you need to access envd directly without SDK and you are now getting 401, you may need to set `secure=False`.
...
// Or get all at once
const allSandboxes = list(paginator)
...
# Iterate through all sandboxes
for sandbox in paginator:
    print(sandbox.sandbox_id)

# Or get all at once
all_sandboxes = list(paginator)
