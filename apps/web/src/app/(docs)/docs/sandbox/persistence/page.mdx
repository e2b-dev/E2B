...
**Note: Auto-pause is currently in beta and available through `betaCreate`/`beta_create()` method.**

Sandboxes can now automatically pause after they times out. When a sandbox is paused, it stops consuming compute but preserves its state. The default inactivity timeout is 10 minutes.
...
- Sandbox can be used up to 30 days
  - After 30 days from the sandbox start, the data may be deleted and you will not be able to resume it. Attempting to resume a sandbox that was deleted or does not exist will result in the `NotFoundError` in the JavaScript SDK or the `NotFoundException` in the Python SDK