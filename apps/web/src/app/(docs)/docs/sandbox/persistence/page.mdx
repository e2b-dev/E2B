...
const sameSbx = await sbx.betaConnect() // $HighlightLine
...
# Connect to the sandbox (it will automatically resume the sandbox, if paused)
same_sbx = sbx.beta_connect() # $HighlightLine
...
You can pass a custom timeout to the `Sandbox.betaConnect()`/`Sandbox.beta_connect()` method like this:

<CodeGroup>
```js
import { Sandbox } from '@e2b/code-interpreter'

const sbx = await Sandbox.betaConnect(sandboxId, { timeoutMs: 60 * 1000 }) // 60 seconds
```
```python
from e2b_code_interpreter import Sandbox

sbx = Sandbox.beta_connect(sandbox_id, timeout=60) # 60 seconds
```
</CodeGroup>
