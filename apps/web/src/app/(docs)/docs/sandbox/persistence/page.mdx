...
const sandboxId = await sbx.beta.pause() // $HighlightLine
...
# You can save the sandbox ID in your database
# to resume the sandbox later
sandbox_id = sbx.beta.pause() # $HighlightLine
...
// You can save the sandbox ID in your database
// to resume the sandbox later
const sandboxId = await sbx.beta.pause()
console.log('Sandbox paused', sandboxId)

// Resume the sandbox from the same state
const sameSbx = await sbx.beta.resume() // $HighlightLine
...
# Resume the sandbox from the same state
same_sbx = sbx.beta.resume() # $HighlightLine
...
You can pass a custom timeout to the `Sandbox.beta.resume()` method like this:

<CodeGroup>
```js
import { Sandbox } from '@e2b/code-interpreter'

const sbx = await Sandbox.beta.resume(sandboxId, { timeoutMs: 60 * 1000 }) // 60 seconds
```
```python
from e2b_code_interpreter import Sandbox

sbx = Sandbox.beta.resume(sandbox_id, timeout=60) # 60 seconds
```
</CodeGroup>
