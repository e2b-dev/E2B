...
<Note>
If you're using the [beta version of the SDK](/docs/sandbox/persistence#1-installing-the-beta-version-of-the-sdks), the `Sandbox.list()` method was updated.
</Note>

### Listing sandboxes

The `Sandbox.list()` method now returns a paginator that you can use to iterate over all sandboxes.
...
You can filter sandboxes by state...
...
You can set custom per-page limit (default 1000) and offset (specified by `nextToken` or `next_token`).
