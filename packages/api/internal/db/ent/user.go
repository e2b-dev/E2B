	...
	for i := range columns {
		...
		case user.ForeignKeys[0]:
			if value, ok := values[i].(*sql.NullInt64); !ok {
				return fmt.Errorf("unexpected type %T for edge-field access_token_users", value)
			} else if value.Valid {
				u.access_token_users = new(int)
				*u.access_token_users = int(value.Int64)
			}
		default:
