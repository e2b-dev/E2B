...
test.each([
  ...
  async ({ startupError, cleanupError }) => {
    ...
    expect(error.message).toContain(sandbox.sandboxId)
    expect(error.message).toContain(`Sandbox.kill('${sandbox.sandboxId}')`)
    expect(error.message).not.toContain(String(startupError))
    expect(error.message).not.toContain(String(cleanupError))
