...
test.skipIf(runtime !== 'node')(
  'caches envd fetchers per CA bundle',
  async () => {
    const { createEnvdFetch, createEnvdRpcFetch } = await import(
      '../../src/envd/http2'
    )
    ...
    const plain = createEnvdFetch()
    const proxied = createEnvdFetch({ proxy: 'http://127.0.0.1:8080' })
    ...
    expect(trusting).not.toBe(proxied)

    const rpcPlain = createEnvdRpcFetch()
    const rpcTrusting = createEnvdRpcFetch({ caBundle: '/etc/ssl/ca.pem' })

    expect(createEnvdRpcFetch({ caBundle: '/etc/ssl/ca.pem' })).toBe(
      rpcTrusting
    )
