...
test('dont timeout session', async () => {
  const session = await Session.create({ id: 'Nodejs', apiKey: E2B_API_KEY, timeout: 10000})
  await session.close()
})
...
test('timeout filesystem', async () => {
  ...
  await expect(session.filesystem.read('/home/test.txt', {timeout: 10})).rejects.toThrow(
    TimeoutError,
  )
  await expect(session.filesystem.list('/home', {timeout:10})).rejects.toThrow(TimeoutError)
  await expect(session.filesystem.makeDir('/home/test/', {timeout:10})).rejects.toThrow(
    TimeoutError,
  )
  await session.filesystem.write('/home/test.txt', 'Hello World')
  await expect(session.filesystem.remove('/home/test.txt', {timeout: 10})).rejects.toThrow(
    TimeoutError,
  )
})
...
test('timeout process', async () => {
  ...
  await expect(process.sendStdin('Hello World', {timeout: 10})).rejects.toThrow(TimeoutError)
})
