 ...
 * carries two roles: a `proxy` sidecar (`'iron-proxy'`) that the sandbox's
 * egress is steered through and that swaps a placeholder token for the real
 * secret value on the way out, so the secret never enters the sandbox; and a
 * `service` sidecar (`'redis'`) the sandbox talks to directly. Code in the
 * sandbox reaches a sidecar by the name `{entry}.sidecar.e2b.local`, never by
 * IP.
 *
...
function buildSidecarsBody(
  ...
  return sidecars.map((sidecar, i) => {
    if (!isPlainObject(sidecar) || typeof sidecar.entry !== 'string') {
      throw new InvalidArgumentError(
        `sidecars[${i}] must be an object with a string 'entry' naming a catalog entry (e.g. 'redis').`
      )
    }
