...
from e2b_desktop import E2B, Sandbox
...
def api_server(monkeypatch):
    ...
    # does not exist for a mocked API. All the API calls carrying the bound
    # configuration happen before it.
    monkeypatch.setattr(Commands, "run", lambda *args, **kwargs: _StubHandle())
    monkeypatch.setattr(Sandbox, "_wait_and_verify", lambda *args, **kwargs: True)
    monkeypatch.setattr(Sandbox, "_start_xfce4", lambda self: None)
...
def test_client_sandbox_keeps_the_desktop_api(api_server):
    client = E2B(api_key=API_KEY_A, domain=DOMAIN_A, api_url=api_server)

    assert issubclass(client.Sandbox, Sandbox)
    assert client.Sandbox is not Sandbox
    # Class-level defaults are inherited.
    assert client.Sandbox.default_template == "desktop"

    sandbox = client.Sandbox.create()

    assert isinstance(sandbox, client.Sandbox)
    assert isinstance(sandbox, Sandbox)
    assert callable(sandbox.screenshot)
    assert callable(sandbox.stream.start)
...
def test_top_level_classes_keep_using_the_env_config(api_server):
    ...
    # Creating clients does not bind anything onto the top-level class.
