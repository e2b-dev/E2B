...
func generate(gen *Plugin, file *descriptor.FileDescriptorProto) {
	...
	print(b, "from typing import Any, Generator, Coroutine, AsyncGenerator")
	...
	for _, svc := range file.Service {
		...
		print(b, "    def __init__(self, base_url: str, *, pool: ConnectionPool, async_pool: AsyncConnectionPool, compressor=None, json=False, **opts):")
		if len(svc.Method) == 0 {
			print(b, "        pass")
			continue
		}
