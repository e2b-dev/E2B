...
class CodeSnippetManager(BaseModel):
    _service_name: ClassVar[str] = 'codeSnippet'
    session_connection: SessionConnection = PrivateAttr()
    opts: Optional[CodeSnippetOpts]

    async def run(self, code: str, envVars: Optional[EnvVars] = None) -> CodeSnippetExecState:
        state: CodeSnippetExecState = await self.session_connection.call(self._service_name, 'run', [code, envVars])
        if self.opts and self.opts.onStateChange:
            self.opts.onStateChange(state)
        return state

    async def stop(self) -> CodeSnippetExecState:
        state: CodeSnippetExecState = await self.session_connection.call(self._service_name, 'stop')
        if self.opts and self.opts.onStateChange:
            self.opts.onStateChange(state)
        return state
    ...
    async def _subscribe(self):
      ...
      await self.session_connection.handle_subscriptions(
            self.session_connection.subscribe(self._service_name, self.opts.onStateChange, 'state') if self.opts.onStateChange else None,
            self.session_connection.subscribe(self._service_name, self.opts.onStderr, 'stderr') if self.opts.onStderr else None,
            self.session_connection.subscribe(self._service_name, self.opts.onStdout, 'stdout') if self.opts.onStdout else None,
            self.session_connection.subscribe(self._service_name, self.opts.onScanPorts, 'scanOpenedPorts') if self.opts.onScanPorts else None,
      )
