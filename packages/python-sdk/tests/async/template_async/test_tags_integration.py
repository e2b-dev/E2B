...
async def test_build_template_with_tags_and_manage_them():
    """Test building a template with tags and managing them."""
    template_alias = f"e2b-async-tags-test-{test_run_id}"
    ...
    tag_info = await AsyncTemplate.assign_tag(initial_tag, [production_tag, latest_tag])
    ...
    with pytest.raises(TemplateException):
        await AsyncTemplate.delete_tag(
            f"{template_alias}:nonexistent-tag-{test_run_id}"
        )
...
@pytest.mark.asyncio
@pytest.mark.skip_debug()
async def test_assign_single_tag_to_existing_template():
    """Test assigning a single tag (not array) to existing template."""
    template_alias = f"e2b-async-single-tag-test-{test_run_id}"
    initial_tag = f"{template_alias}:v1.0"
