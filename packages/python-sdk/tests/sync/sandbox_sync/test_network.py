...
def test_transform_callable_sees_every_registered_iam_token():
    ...
    network: SandboxNetworkOpts = {
        "rules": {
            "api.internal.example.com": [
                {
                    "transform": lambda ctx: {
                        "headers": {"X-Tokens": ",".join(ctx.iam.tokens)},
                    },
                },
            ],
        },
    ...
    assert body["rules"].to_dict() == {
        "api.internal.example.com": [
            {"transform": {"headers": {"X-Tokens": "aws,gcp"}}},
        ],
    }
...
def test_transform_callable_rejects_unregistered_iam_token():
    iam = build_iam_config(
        {
            "tokens": {
                "aws": Secret.iam_token(
            ...
            "api.internal.example.com": [
                {
                    "transform": lambda ctx: {
                        "headers": {
                            "Authorization": f"Bearer {ctx.iam.tokens['awz']}",
                        },
                    },
                },
            ],
