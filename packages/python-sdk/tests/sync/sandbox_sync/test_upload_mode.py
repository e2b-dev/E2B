"""The octet-stream-vs-multipart upload decision, asserted on the request the
SDK sends to a mocked envd file API. Mirrors `tests/sandbox/uploadMode.test.ts`.
"""
...
def test_in_memory_data_uploads_as_multipart(test_api_key):
    requests: List[httpx.Request] = []
    filesystem = _filesystem(test_api_key, requests)

    filesystem.write("/home/user/a.txt", "hello")

    assert requests[0].headers["Content-Type"].startswith("multipart/form-data")
    assert requests[0].url.params["path"] == "/home/user/a.txt"


def test_octet_stream_can_be_requested_explicitly(test_api_key):
    requests: List[httpx.Request] = []
    filesystem = _filesystem(test_api_key, requests)

    filesystem.write("/home/user/a.txt", "hello", use_octet_stream=True)

    assert requests[0].headers["Content-Type"] == "application/octet-stream"
    assert requests[0].content == b"hello"


def test_file_like_data_defaults_to_octet_stream(test_api_key):
    requests: List[httpx.Request] = []
    filesystem = _filesystem(test_api_key, requests)

    filesystem.write("/home/user/a.txt", io.BytesIO(b"hello"))

    assert requests[0].headers["Content-Type"] == "application/octet-stream"


def test_octet_stream_falls_back_to_multipart_on_old_envd(test_api_key):
    requests: List[httpx.Request] = []
...
def test_gzip_implies_octet_stream_and_sets_content_encoding(test_api_key):
    requests: List[httpx.Request] = []
    filesystem = _filesystem(test_api_key, requests)

    filesystem.write("/home/user/a.txt", "hello", gzip=True)
...
def test_metadata_is_sent_as_headers(test_api_key):
    requests: List[httpx.Request] = []
    filesystem = _filesystem(test_api_key, requests)

    filesystem.write("/home/user/a.txt", "hello", metadata={"origin": "test"})

    assert requests[0].headers["X-Metadata-origin"] == "test"


def test_multi_file_multipart_upload_omits_path_param(test_api_key):
    requests: List[httpx.Request] = []
    filesystem = _filesystem(test_api_key, requests)
