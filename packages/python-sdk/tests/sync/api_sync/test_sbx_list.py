...
def test_list_sandboxes(sandbox: Sandbox, sandbox_type: str):
    paginator = Sandbox.list(
        query=SandboxListQuery(metadata={"sandbox_type": sandbox_type})
    )
...
def test_list_sandboxes_with_filter(sandbox_type: str):
    ...
    extra_sbx = Sandbox(metadata={"unique_id": unique_id}, auto_pause=True)

    try:
        paginator = Sandbox.list(
            query=SandboxListQuery(metadata={"unique_id": unique_id})
        )
        sandboxes = paginator.next_items()
        assert len(sandboxes) == 1
...
def test_list_running_sandboxes(sandbox: Sandbox, sandbox_type: str):
    paginator = Sandbox.list(
        query=SandboxListQuery(
            metadata={"sandbox_type": sandbox_type}, state=["running"]
        )
    )
    sandboxes = paginator.next_items()
    assert len(sandboxes) >= 1
    ...
    assert any(
        s.sandbox_id == sandbox.sandbox_id and s.state == "running" for s in sandboxes
    )
...
def test_list_paused_sandboxes(sandbox: Sandbox, sandbox_type: str):
    ...
    paginator = Sandbox.list(
        query=SandboxListQuery(
            metadata={"sandbox_type": sandbox_type}, state=["paused"]
        )
    )
    sandboxes = paginator.next_items()
    assert len(sandboxes) >= 1
...
def test_paginate_running_sandboxes(sandbox: Sandbox, sandbox_type: str):
    ...
    extra_sbx = Sandbox(metadata={"sandbox_type": sandbox_type}, auto_pause=True)
...
def test_paginate_paused_sandboxes(sandbox: Sandbox, sandbox_type: str):
    ...
    try:
        ...
        paginator = Sandbox.list(
            query=SandboxListQuery(
                state=["paused"], metadata={"sandbox_type": sandbox_type}
            ),
            limit=1,
        )
...
def test_paginate_running_and_paused_sandboxes(sandbox: Sandbox, sandbox_type: str):
    ...
    try:
        ...
        paginator = Sandbox.list(
            query=SandboxListQuery(
                metadata={"sandbox_type": sandbox_type}, state=["running", "paused"]
            ),
            limit=1,
        )
