...
class AsyncSandbox(SandboxApi):
    ...
    async def create(
        ...
        **opts: Unpack[ApiParams],
        ...
        :param iam: Sandbox workload identity configuration. A non-empty ``tokens`` map enables workload identity for the sandbox; token definitions can be created with :meth:`Secret.id_token`. Example: ``{"tokens": {"aws": Secret.id_token(audience="sts.amazonaws.com", token_type="JWT-SVID")}}``
